	...
	<form action="<?php echo BASE_URL ?>post/update_post/<?php echo $pos['id_post'] ?>" method="POST" enctype="multipart/form-data"	>
	  ...
	  <div class="form-group">
	    ...
	    <select name="category_post" class="form-control">
	    	...
	    	?>
			<option <?php if($cate['id_category_post']==$pos['id_category_post']) echo 'selected'; else echo '' ?> value="<?php echo $cate['id_category_post']?>"><?php echo $cate['title_category_post']?></option>	    	
			<?php
	    	}
	    	?>
	    </select>
